...
- 🛎️ **Service catalogue** — each clinic manages its bookable procedures by **dental area** (name, area, duration, price) and assigns **which dentists perform each one**; a starter set is seeded on registration. Duration drives the online slot length.
- 🌐 **Online booking (public marketplace)** — a no-auth directory lists every clinic; patients open a clinic, pick a **service** (grouped by area), then only the **dentists who perform it**, a free slot and request an appointment. The request lands as `PENDING` and reserves the slot (reusing the overlap check) until the clinic confirms or rejects it; pending requests are surfaced on the dashboard.
