...
- 🛎️ **Catálogo de serviços** — cada clínica gerencia seus procedimentos por **área da odontologia** (nome, área, duração, preço) e define **quais dentistas realizam cada um**; um conjunto inicial é semeado no cadastro. A duração define o tamanho do horário no agendamento online.
- 🌐 **Agendamento online (marketplace público)** — um diretório sem login lista todas as clínicas; o paciente abre uma clínica, escolhe um **serviço** (agrupado por área), então apenas os **dentistas que o realizam**, um horário livre e solicita a consulta. A solicitação entra como `PENDENTE` e reserva o horário (reusando a checagem de sobreposição) até a clínica confirmar ou recusar; as solicitações pendentes aparecem no dashboard.
