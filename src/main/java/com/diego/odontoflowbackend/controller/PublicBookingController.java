...
/** Public, unauthenticated online-booking endpoints (marketplace + per-clinic booking). */
@RestController
...
public class PublicBookingController {
    ...
    @GetMapping
    @Operation(summary = "Directory of all clinics (marketplace)")
    public List<ClinicSummaryResponse> clinics() {
        return publicBookingService.clinics();
    }

    @GetMapping("/{slug}")
    @Operation(summary = "Public clinic profile (name + bookable dentists + services)")
    public PublicClinicResponse clinic(@PathVariable String slug) {
        return publicBookingService.clinic(slug);
    }

    @GetMapping("/{slug}/availability")
    @Operation(summary = "Free start times for a dentist + service on a given day")
    public AvailabilityResponse availability(
            @PathVariable String slug,
            @RequestParam UUID dentistId,
            @RequestParam UUID serviceId,
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date) {
        return publicBookingService.availability(slug, dentistId, serviceId, date);
    }
