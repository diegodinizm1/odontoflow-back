...
/**
 * Unauthenticated, tenant-scoped-by-slug booking flow for the public page.
 * A booking is created as {@link AppointmentStatus#PENDING}: it reserves the slot
 * (the overlap check counts PENDING) until the clinic confirms or rejects it.
 */
@Service
...
public class PublicBookingService {
    ...
    public PublicClinicResponse clinic(String slug) {
        ...
        List<PublicDentistResponse> dentists = userRepository
                ...
                .toList();
        return new PublicClinicResponse(tenant.getClinicName(), tenant.getPublicSlug(), dentists);
    }

    public AvailabilityResponse availability(String slug, UUID dentistId, LocalDate date) {
        Tenant tenant = getTenant(slug);
        ...
        for (LocalTime t = LocalTime.of(WORK_START_HOUR, 0);
             !t.plusMinutes(SLOT_MINUTES).isAfter(LocalTime.of(WORK_END_HOUR, 0));
    ...
    private void validateWithinWorkingHours(LocalTime start, LocalTime end) {
        ...
        boolean offGrid = start.getMinute() % SLOT_MINUTES != 0;
